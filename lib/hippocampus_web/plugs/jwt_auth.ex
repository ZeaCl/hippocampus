defmodule HippocampusWeb.Plugs.JWTAuth do
    ...
    case get_req_header(conn, "authorization") do
      ["Bearer " <> token] ->
        case validate_jwt(token) do
          ...
          {:error, reason} ->
            conn |> json_error(401, reason) |> halt()
        end

      _ ->
        conn  # No JWT — let ApiKeyPlug try next
    end
  end
  ...
  defp validate_jwt(token) do
    with {:ok, jwks} <- fetch_jwks(),
      ...
      {:error, reason} -> {:error, "JWT validation failed: #{inspect(reason)}"}
    end
  rescue
  ...
  defp fetch_jwks do
    jwks_url = Application.get_env(:hippocampus, :thalamus)[:jwks_url]
